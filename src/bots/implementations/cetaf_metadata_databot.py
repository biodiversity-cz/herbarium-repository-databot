...
class CetafMetadataDatabot(AbstractUrlDatabot):
    ...
    def get_url(self, record: dict) -> str:
        pid = record.get("specimen_pid")
        if not pid:
            raise ValueError(f"Record {record.get('id')} has no specimen_pid to harvest")
        return pid
    ...
    def run(self):
        # Žádné spojení se nezavírá - spojení patří connection poolu a vracejí se
        # po jednotlivých dotazech.
